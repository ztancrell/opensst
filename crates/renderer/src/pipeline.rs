...
/// Create bind group layout for terrain (camera + terrain uniform + deformation texture).
...
/// Create water render pipeline (lakes, streams, ocean). Uses terrain bind group.
pub fn create_water_pipeline(
    ...
    device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        ...
        primitive: wgpu::PrimitiveState {
            ...
            strip_index_format: None,
            front_face: wgpu::FrontFace::Ccw,
            cull_mode: None, // Double-sided: visible from above AND when submerged looking up
            polygon_mode: wgpu::PolygonMode::Fill,
            unclipped_depth: false,
            conservative: false,
        },
...
/// Create bind group layout for sky (camera + sky uniform).
pub fn create_sky_bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
...
/// Create sky render pipeline (fullscreen triangle, no vertex buffer).
...
/// Create texture bind group layout for PBR materials.
...
/// Create the overlay bind group layout (font texture + sampler).
...
/// Create the screen-space overlay pipeline for text / UI.
pub fn create_overlay_pipeline(
    ...
    device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        ...
        vertex: wgpu::VertexState {
            module: &shader,
            entry_point: Some("vs_main"),
            buffers: &[crate::vertex::OverlayVertex::layout()],
            compilation_options: Default::default(),
        },
        fragment: Some(wgpu::FragmentState {
            module: &shader,
        ...
        depth_stencil: None, // No depth test for screen-space overlay
        multisample: wgpu::MultisampleState {
            count: 1,
            mask: !0,
            alpha_to_coverage_enabled: false,
...
/// Create celestial body render pipeline (instanced sphere rendering for stars/planets/moons).
pub fn create_celestial_pipeline(
    device: &wgpu::Device,
    config: &wgpu::SurfaceConfiguration,
    camera_bind_group_layout: &wgpu::BindGroupLayout,
) -> wgpu::RenderPipeline {
    ...
    device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        ...
        vertex: wgpu::VertexState {
            module: &shader,
            entry_point: Some("vs_main"),
            buffers: &[Vertex::layout(), CelestialBodyInstance::layout()],
            compilation_options: Default::default(),
        },
        fragment: Some(wgpu::FragmentState {
            module: &shader,
...
/// Bind group layout for bright pass: scene texture, sampler, uniform.
pub fn create_bright_bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
...
/// Bind group layout for blur pass: input texture, sampler, uniform.
...
/// Bind group layout for cinematic post-process: scene, bloom, depth, samplers, uniform.
pub fn create_cinematic_bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("Cinematic Bind Group Layout"),
        entries: &[
            wgpu::BindGroupLayoutEntry {
                binding: 0,
                visibility: wgpu::ShaderStages::FRAGMENT,
...
/// Cinematic fullscreen pass: color grading, film grain, vignette (97 movie / Heinlein look).
pub fn create_cinematic_pipeline(
    device: &wgpu::Device,
    config: &wgpu::SurfaceConfiguration,
    cinematic_bind_group_layout: &wgpu::BindGroupLayout,
) -> wgpu::RenderPipeline {
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Cinematic Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("shaders/cinematic.wgsl").into()),
    });

    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("Cinematic Pipeline Layout"),
        bind_group_layouts: &[cinematic_bind_group_layout],
        push_constant_ranges: &[],
    });

    device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("Cinematic Pipeline"),
        layout: Some(&pipeline_layout),
        vertex: wgpu::VertexState {
            module: &shader,
            entry_point: Some("vs_main"),
...
/// Bright pass pipeline: extract bright pixels for bloom.
pub fn create_bright_pipeline(
    device: &wgpu::Device,
    config: &wgpu::SurfaceConfiguration,
    bright_bind_group_layout: &wgpu::BindGroupLayout,
) -> wgpu::RenderPipeline {
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Bright Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("shaders/bright.wgsl").into()),
    });
    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
...
/// Blur pipeline: separable Gaussian blur.
